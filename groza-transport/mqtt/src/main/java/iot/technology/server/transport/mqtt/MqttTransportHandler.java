...
import iot.technology.server.common.data.security.DeviceTokenCredentials;
import iot.technology.server.common.data.security.DeviceX509Credentials;
import iot.technology.server.common.msg.core.SessionOpenMsg;
import iot.technology.server.common.msg.session.AdaptorToSessionActorMsg;
import iot.technology.server.common.msg.session.BasicAdaptorToSessionActorMsg;
import iot.technology.server.common.msg.session.BasicTransportToDeviceSessionActorMsg;
...
import iot.technology.server.common.transport.quota.QuotaService;
...
import iot.technology.server.transport.mqtt.session.GatewaySessionCtx;
import iot.technology.server.transport.mqtt.util.SslUtil;
...
import static iot.technology.rule.engine.producers.producers.PulsarClient;
    ...
    private void processDevicePublish(ChannelHandlerContext ctx, MqttPublishMessage mqttMsg, String topicName, int msgId) {
        ...
        try {
            if (topicName.equals(MqttTopics.DEVICE_TELEMETRY_TOPIC)) {
                msg = adaptor.convertToActorMsg(deviceSessionCtx, SessionMsgType.POST_TELEMETRY_REQUEST, mqttMsg);
            } else if (topicName.equals(MqttTopics.DEVICE_ATTRIBUTES_TOPIC)) {
                msg = adaptor.convertToActorMsg(deviceSessionCtx, SessionMsgType.POST_ATTRIBUTES_REQUEST, mqttMsg);
            } else if (topicName.startsWith(MqttTopics.DEVICE_ATTRIBUTES_REQUEST_TOPIC_PREFIX)) {
                msg = adaptor.convertToActorMsg(deviceSessionCtx, SessionMsgType.GET_ATTRIBUTES_REQUEST, mqttMsg);
                if (msgId >= 0) {
                    ctx.writeAndFlush(createMqttPubAckMsg(msgId));
                }
            } else if (topicName.startsWith(MqttTopics.DEVICE_RPC_RESPONSE_TOPIC)) {
                msg = adaptor.convertToActorMsg(deviceSessionCtx, SessionMsgType.TO_DEVICE_RPC_RESPONSE, mqttMsg);
                if (msgId >= 0) {
                    ctx.writeAndFlush(createMqttPubAckMsg(msgId));
                }
            } else if (topicName.startsWith(MqttTopics.DEVICE_RPC_REQUESTS_TOPIC)) {
                msg = adaptor.convertToActorMsg(deviceSessionCtx, SessionMsgType.TO_SERVER_RPC_REQUEST, mqttMsg);
                if (msgId >= 0) {
                    ctx.writeAndFlush(createMqttPubAckMsg(msgId));
                }
            }
            ...
            try {
                if (topicName.equals(MqttTopics.DEVICE_ATTRIBUTES_TOPIC)) {
                    AdaptorToSessionActorMsg msg =
                            adaptor.convertToActorMsg(deviceSessionCtx, SessionMsgType.SUBSCRIBE_ATTRIBUTES_REQUEST, mqttMsg);
//                    processor.process(new BasicTransportToDeviceSessionActorMsg(deviceSessionCtx.getDevice(), msg));
                    grantedQoSList.add(getMinSupportedQos(reqQoS));
                } else if (topicName.equals(MqttTopics.DEVICE_RPC_REQUESTS_SUB_TOPIC)) {
                    AdaptorToSessionActorMsg msg =
                            adaptor.convertToActorMsg(deviceSessionCtx, SessionMsgType.SUBSCRIBE_RPC_COMMANDS_REQUEST, mqttMsg);
//                    processor.process(new BasicTransportToDeviceSessionActorMsg(deviceSessionCtx.getDevice(), msg));
                    grantedQoSList.add(getMinSupportedQos(reqQoS));
    ...
    private void processUnsubscribe(ChannelHandlerContext ctx, MqttUnsubscribeMessage mqttMsg) {
        ...
        for (String topicName : mqttMsg.payload().topics()) {
            try {
                if (topicName.equals(MqttTopics.DEVICE_ATTRIBUTES_TOPIC)) {
                    AdaptorToSessionActorMsg msg =
                            adaptor.convertToActorMsg(deviceSessionCtx, SessionMsgType.UNSUBSCRIBE_ATTRIBUTES_REQUEST, mqttMsg);
//                    processor.process(new BasicTransportToDeviceSessionActorMsg(deviceSessionCtx.getDevice(), msg));
                } else if (topicName.equals(MqttTopics.DEVICE_RPC_REQUESTS_SUB_TOPIC)) {
                    AdaptorToSessionActorMsg msg =
                            adaptor.convertToActorMsg(deviceSessionCtx, SessionMsgType.UNSUBSCRIBE_RPC_COMMANDS_REQUEST, mqttMsg);
//                    processor.process(new BasicTransportToDeviceSessionActorMsg(deviceSessionCtx.getDevice(), msg));
                } else if (topicName.equals(MqttTopics.DEVICE_ATTRIBUTES_RESPONSES_TOPIC)) {
