package iot.technology.server.dao.asset;
...
import iot.technology.server.common.data.asset.AssetSearchQuery;
...
public class AssetServiceImpl extends AbstractEntityService implements AssetService {
    ...
    public Asset findAssetById(AssetId assetId) {
        ...
        Validator.validateId(assetId, INCORRECT_ASSET_ID + assetId);
    ...
    public ListenableFuture<List<Asset>> findAssetsByTenantIdAndIdsAsync(TenantId tenantId, List<AssetId> assetIds) {
        ...
        return assetDao.findAssetsByTenantIdAndIdsAsync(tenantId.getId(), DaoUtil.toUUIDs(assetIds));
    }
    ...
    public ListenableFuture<List<Asset>> findAssetsByTenantIdCustomerIdAndIdsAsync(TenantId tenantId, CustomerId customerId, List<AssetId> assetIds) {
        ...
        Validator.validateId(customerId, INCORRECT_CUSTOMER_ID + customerId);
        Validator.validateIds(assetIds, "Incorrect assetIds " + assetIds);
        return assetDao.findAssetsByTenantIdAndCustomerIdAndIdsAsync(tenantId.getId(), customerId.getId(), DaoUtil.toUUIDs(assetIds));
    }
